...
function getGreeting(now = new Date()) {
  ...
  if (h < 12) {
    return {
      text: "Good Morning",
      icon: <Sunrise className="h-5 w-5" style={{ color: "var(--gold-primary)" }} />,
    };
  }
  ...
  if (h < 18) {
    return {
      text: "Good Afternoon",
      icon: <Sun className="h-5 w-5" style={{ color: "var(--gold-bright)" }} />,
    };
  }

  return {
    text: "Good Evening",
    icon: <Moon className="h-5 w-5" style={{ color: "var(--gold-muted)" }} />,
  };
}

const pieColors = ["#C9A15D", "#E8C27A", "#8A6D3B", "#7A5A2E", "#F5EDE0"];
...
export default function DashboardClient({ initial, user }: Props) {
  ...
  const kpis: KPI[] = useMemo(() => {
    return [
      {
        title: "Total Revenue",
        value: `Rs ${formatMoney(Number(kpiData.revenue) || 0)}`,
        positive: true,
      },
      {
        title: "Avg. Transaction Value",
        value: `Rs ${formatMoney(Number(kpiData.avgOrderValue) || 0)}`,
        positive: true,
      },
      {
        title: "Total Orders",
  ...
  const cardBorder = { border: "1px solid var(--border-subtle)" };
  const cardBg = { backgroundColor: "var(--bg-secondary)" };
  const upColor = "var(--gold-primary)";
  const downColor = "#E57373";
        ...
        <div className="grid gap-4 md:grid-cols-12">
          ...
          <div className="md:col-span-9">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {kpis.map((k) => (
                <div key={k.title} className="rounded-2xl p-4" style={{ ...cardBg, ...cardBorder }}>
                  <div className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>
                    {k.title}
                  </div>
                  <div className="mt-2 flex items-end justify-between gap-3">
                    <div className="text-2xl font-bold" style={{ color: "var(--gold-primary)" }}>
                      {k.value}
                    </div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-12">
            ...
            <Card
              title={
                <div className="flex items-center gap-2">
                  <span style={{ color: "var(--text-primary)" }}>Overall Earnings</span>
                  <span className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>
                    - {rangeMode === "Daily" ? "This Week" : rangeMode}
                  </span>
                  <span className="ml-1 text-sm font-bold" style={{ color: "var(--gold-primary)" }}>
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={earnings}>
                    ...
                    />
                    <Line type="monotone" dataKey="value" strokeWidth={3} dot={false} stroke="#C9A15D" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
          ...
          <div className="md:col-span-4">
            <Card
              ...
              right={<span className="text-xs" style={{ color: "var(--text-secondary)" }}>in selected range</span>}
              className="p-5"
            >
                ...
                {categories.map((c: any, idx: number) => (
                  <div key={c.name} className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <TrendingUp className="h-4 w-4" style={{ color: upColor }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3">
                {driverCards.map((s) => (
                  <div
                    ...
                    style={{ backgroundColor: "var(--bg-elevated)", ...cardBorder }}
                  >
                    <div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {s.up ? (
                        ...
                        <TrendingDown className="h-4 w-4" style={{ color: downColor }} />
                      )}
                    </div>
                  </div>
                ))}
                ...
                {!driverCards.length ? (
                  <div
                    ...
                    style={{ ...cardBorder, backgroundColor: "var(--bg-secondary)", color: "var(--text-secondary)" }}
                  >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{ backgroundColor: "var(--bg-secondary)", color: "var(--gold-primary)" }}
                      >
                        {p.name.slice(0, 1)}
                      </div>
                      <div>
